...
lr = LogisticRegression(tol=0.0001, C=0.2, fit_intercept=True, intercept_scaling=1, class_weight={0: .1, 1:.9}, random_state=0, solver='sag', max_iter=800)
print('fitting')
lr_train = lr.fit(X, y)
print('predicting')
lr_X_prediction  = lr.predict_proba(X)[:, 1]
lr_X_test_prediction  = lr.predict_proba(X_test)[:, 1]
tr_te_concatenated = np.concatenate([lr_X_prediction, lr_X_test_prediction])
df['logistic_regression'] = tr_te_concatenated

print('final tr_te shape', df.shape)
print(df.head())

df.to_csv('logistic_regression_tr_te.csv', index= False)
