...
print('GradientBoostingClassifier Begins****************')
gbc = GradientBoostingClassifier(learning_rate=0.01, n_estimators=1000,
            max_depth=7, min_samples_split=15, random_state=0)

print('fitting..')
gbc_train = gbc.fit(X, y)

print('predicting...')
gbc_X_prediction  = gbc.predict_proba(X)[:, 1]
gbc_X_test_prediction  = gbc.predict_proba(X_test)[:, 1]
tr_te_concatenated = np.concatenate([gbc_X_prediction,gbc_X_test_prediction])
df['gradient_boosting_classifier'] = tr_te_concatenated


df.to_csv('gradient_boosting_classifier_tr_te.csv', index= False)
