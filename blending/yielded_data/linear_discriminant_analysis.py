...
for solver in solvers:
    print('LinearDiscriminantAnalysis solver****************', solver)
    lda = LinearDiscriminantAnalysis(solver=solver)
    print('fitting****************')
    lda_train = lda.fit(X, y)
    print('predicting on train****************')
    lda_X_prediction  = lda.predict_proba(X)[:, 1]
    print('predicting on test****************')
    lda_X_test_prediction  = lda.predict_proba(X_test)[:, 1]
    tr_te_concatenated = np.concatenate([lda_X_prediction,lda_X_test_prediction])
    df['linear_discriminant_analysis_'+ solver + '_solver'] = tr_te_concatenated


print('final tr_te shape', df.shape)
df.to_csv('linear_discriminant_analysis_tr_te.csv', index= False)
print(df.head())
