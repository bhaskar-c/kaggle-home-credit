...
print('GaussianProcessClassifier begins****************')
gpc = GaussianProcessClassifier()
print('fitting****************')
gpc_train = gpc.fit(X, y)
print('predicting****************')
gpc_X_prediction  = gpc.predict_proba(X)[:, 1]
gpc_X_test_prediction  = gpc.predict_proba(X_test)[:, 1]
tr_te_concatenated = np.concatenate([gpc_X_prediction, gpc_X_test_prediction])
df['gaussian_process_classifier'] = tr_te_concatenated
df.to_csv('gaussian_process_classifier_tr_te.csv', index= False)
